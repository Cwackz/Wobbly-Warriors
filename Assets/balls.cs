...
public class ThrowBall : MonoBehaviour
{
    ...
    void PickupBall()
    {
        ...
        // Get the current Z distance from the camera to the ball and use it for the Z axis.
        float zDistance = Camera.main.WorldToScreenPoint(transform.position).z;

        // Update mouse position Z to match the ball's Z depth in world space.
        mousePos.z = zDistance; 

        // Convert the mouse position from screen space to world space
        newPosition = Camera.main.ScreenToWorldPoint(mousePos);

        // Set the ball's world position to follow the mouse smoothly
    ...
    private void Update()
    {
        // Make sure the ball follows the mouse while holding
        if (holding && !thrown)
        {
            PickupBall();
        }
